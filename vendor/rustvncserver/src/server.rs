...
impl VncServer {
    ...
    /// Starts the VNC server, listening for incoming client connections on the specified address.
    ///
    ...
    /// # Arguments
    ///
    /// * `addr` - The socket address(es) to bind, e.g. `"127.0.0.1:5900"`.
    ///   (PaperCast patch: upstream takes a bare port and always binds `0.0.0.0`;
    ///   we need loopback-only binding by default.)
    ///
    /// # Returns
    ///
    ...
    pub async fn listen<A: tokio::net::ToSocketAddrs>(&self, addr: A) -> Result<(), std::io::Error> {
        let listener = TcpListener::bind(addr).await?;
        log::info!("VNC Server listening on {:?}", listener.local_addr());
